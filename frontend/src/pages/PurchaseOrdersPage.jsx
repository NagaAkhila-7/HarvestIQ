...
export const PurchaseOrdersPage = () => {
  ...
  return (
    <div className="space-y-6">
        ...
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-bold uppercase">
            <tr>
              ...
              <th className="p-4">{t('suppliers.leadTime')}</th>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={t('procurement.createPo')}>
        <form onSubmit={handleCreatePO} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5 w-full">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t('procurement.supplier')}</label>
              <select
          ...
          <div className="flex flex-col gap-1.5 w-full">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t('inventory.sku')}</label>
            <select
              value={newPO.itemId}
              onChange={e => {
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label={t('inventory.currentStock')} type="number" required value={newPO.orderedQuantity} onChange={e => setNewPO({...newPO, orderedQuantity: Number(e.target.value)})} />
            <Input label={t('inventory.unitPrice')} type="number" required value={newPO.unitPrice} onChange={e => setNewPO({...newPO, unitPrice: Number(e.target.value)})} />
          </div>
