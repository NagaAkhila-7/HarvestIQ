...
export const ItemDetailPage = () => {
  ...
  if (!data || !data.item) {
    return <div className="p-8 text-center text-rose-500">{t('inventory.noItems')}</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        description={`${t('inventory.category')}: ${item.type} | ${t('inventory.leadTimeDays')}: ${item.leadTimeDays} | ${t('inventory.safetyStock')}: ${item.safetyStock} ${item.unit}`}
        actions={
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="glass-card p-5">
          <div className="text-xs font-bold text-slate-500 uppercase">{t('inventory.currentStock')}</div>
          <div className="text-2xl font-black text-slate-900 dark:text-slate-100 mt-1">
            {currentStock} {item.unit}
          </div>
        </div>
        <div className="glass-card p-5">
          <div className="text-xs font-bold text-slate-500 uppercase">{t('inventory.reorderPoint')}</div>
          <div className="text-2xl font-black text-amber-600 dark:text-amber-400 mt-1">
            {reservedQuantity} {item.unit}
          </div>
        </div>
        <div className="glass-card p-5">
          <div className="text-xs font-bold text-slate-500 uppercase">{t('inventory.safetyStock')}</div>
      ...
      <div className="glass-card p-6">
        <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 mb-4">{t('receiving.lotNumber')}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-bold">
              <tr>
                <th className="p-3">{t('receiving.lotNumber')}</th>
                <th className="p-3">Expiry Date</th>
                <th className="p-3">{t('inventory.currentStock')}</th>
                <th className="p-3">{t('inventory.currentStock')}</th>
                <th className="p-3">{t('receiving.qualityStatus')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {lots.length === 0 ? (
                <tr><td colSpan={5} className="p-4 text-slate-500">{t('inventory.noItems')}</td></tr>
              ) : (
                lots.map(lot => (
                  <tr key={lot._id}>
      ...
      {/* Adjustment Modal */}
      <Modal isOpen={isAdjustModalOpen} onClose={() => setIsAdjustModalOpen(false)} title={t('inventory.editItem')}>
        <form onSubmit={handleStockAdjustment} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
            <Button type="button" variant="outline" onClick={() => setIsAdjustModalOpen(false)}>{t('inventory.cancel')}</Button>
            <Button type="submit" variant="primary">{t('inventory.saveChanges')}</Button>
          </div>
        </form>
      </Modal>
    </div>
